...
def test_send_order_confirmation_email(mocker):
    ...
    rts = mocker.patch(
        "apps.order.utils.render_to_string", return_value="<html>email</html>"
    )
    ...
    outbox = mail.outbox  # type:ignore

    rts.assert_called_with("emails/order_confirmation_email.html", context)
    assert outbox[0].to == [order.user.email]
    assert outbox[0].from_email == settings.DEFAULT_FROM_EMAIL
    assert outbox[0].subject == "[E-commerce] Confirmed order"
    assert outbox[0].alternatives[0][0] == "<html>email</html>"
...
def test_report_yesterday_orders_to_staff_email(mocker):
    ...
    orders_w_items = baker.make("order.Order", 2, created_at=yesterday, paid_amount=5)
    orders_w_same_user = baker.make(
        "order.Order", 2, created_at=yesterday, paid_amount=5, user=user
    )
    ...
    context = {"date": date, "total": 20.00, "num_items": 8, "num_diff_users": 3}

    outbox = mail.outbox  # type: ignore

    rts.assert_called_with("emails/daily_report.html", context)
    assert outbox[0].to == ["t1@ex.com", "t2@ex.com"]
    assert outbox[0].from_email == settings.DEFAULT_FROM_EMAIL
    assert outbox[0].subject == "[E-commerce] " + date + " report"
    assert outbox[0].alternatives[0][0] == "<html>email</html>"
